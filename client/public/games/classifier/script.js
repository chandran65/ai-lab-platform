    ...
    function saveExperimentRecord(summary, datasetSize, algos) {
      ...
      try {
        ...
        state.experiments.forEach(exp => {
          Object.values(exp.results).forEach(res => {
            if (res.accuracy && res.accuracy > bestAcc) {
        ...
        window.parent.postMessage({
          type: 'GAME_PROGRESS',
          gameId: 'image_classifier',
          progressData: {
            modelsTrained: state.experiments.length * state.selectedAlgos.length,
            bestAccuracy: Math.round(bestAcc * 100),
            experimentsRun: state.experiments.length
          }
        }, '*');
      } catch (err) {
